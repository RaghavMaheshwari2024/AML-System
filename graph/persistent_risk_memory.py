...
class PersistentRiskMemory:
    def update(self, memory, risk_scores, communities):
        for node in risk_scores:
            current_risk = risk_scores[node]
            community = communities.get(node, {})
            community_risk = community.get("community_avg_risk", 0.0)
            memory_score = ALPHA * current_risk + BETA * community_risk
            
            memory[node] = {
                "memory_score": memory_score,
                "current_risk": current_risk,
                "community_id": community.get("community_id", -1),
                "community_size": community.get("community_size", 1),
                "community_density": community.get("community_density", 0.0),
                "community_avg_risk": community.get("community_avg_risk", 0.0),
                "community_max_risk": community.get("community_max_risk", 0.0)
            }
        return memory
...
if __name__ == "__main__":
    ...
    for node, info in top_nodes:
        print(f"{node} : {info['memory_score']:.4f}")